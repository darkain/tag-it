...
(function($) {
	...
	$.widget('ui.tagit', {
		...
		_create: function() {
			...
			var that = this;
			var ps = this.options.parentSelector.split(',')[0].trim();
			...
			if (this.element.is('input')) {
				this.tagList = $('<'+ps+'></'+ps+'>').insertAfter(this.element);
				this.options.singleField = true;
				this.options.singleFieldNode = this.element;
				this.element.addClass('tagit-hidden-field');
			} else {
				this.tagList = this.element.find(this.parentSelector).andSelf().last();
			}
			...
			this.tagList
				...
				.append($('<'+cs+' class="tagit-new"></'+cs+'>').append(this.tagInput))
				.click(function(e) {
					var target = $(e.target);
		...
		destroy: function() {
			...
			if (this.element.is('input')) {
				...
				this.element.children(this.options.childSelector).each(function() {
					if ($(this).hasClass('tagit-new')) {
						$(this).remove();
					} else {
						$(this).removeClass([
		...
		createTag: function(value, additionalClass, duringInitialization) {
			...
			if (this.options.readOnly){
				...
				// \xd7 is an X - MULTIPLICATION SIGN
